...
2. **Directory Architecture:** All files MUST be placed according to [`21_FOLDER_STRUCTURE.md`](./docs/Technical%20Design/21_FOLDER_STRUCTURE.md):
   * Shared Configs -> `src/ReplicatedStorage/Shared/Config/`
   * Server Services -> `src/ServerScriptService/Server/Services/`
   * Server Boot -> `src/ServerScriptService/Server/Boot/ServerLoader.server.lua`
   * Client Controllers -> `src/StarterPlayer/StarterPlayerScripts/Client/Controllers/`
   * Client Boot -> `src/StarterPlayer/StarterPlayerScripts/Client/Boot/ClientLoader.client.lua`
...
* [x] **Persistence:** `SaveService.lua` (DataStore profile loading/saving verified).
* [x] **Server Cultivation:** `CultivationService.lua` (Grounded HP math, Qi ticks, breakthrough triggers).
* [x] **Client Cultivation:** `CultivationController.lua` (Smooth 0.35s Sine float-up, instant 0.0s exit landing, 0.3s input debounce, race-condition session tokens, dynamic realm aura highlight & particles).
* [x] **Client UI:** `UIController.lua` (Real-time Qi bar, health updates, reset stats testing button).

### 3.2 Currently In Progress (PHASE 1 - TASK 1.3)
* [ ] **DataStore Schema Expansion & ProfileService Session Locking:** Upgrading `SaveService.lua` to full `ProfileService` with session locking to prevent multi-server item dupes per [`24_SAVE_SYSTEM.md`](./docs/Technical%20Design/24_SAVE_SYSTEM.md).

### 3.3 Next Priority Queue
1. Upgrade `SaveService.lua` to official `ProfileService` session-locking standard.
2. Build `HerbNodeComponent.lua` & `QiNodeComponent.lua` in `Server/Components/` for world harvesting interactions.
3. Build `WorldController.lua` for zone boundary detection and ambient fog/miasma visual alerts.
...
| :--- | :--- |
| **Cultivation & Breakthroughs** | [`docs/Game Design/03_CULTIVATION.md`](./docs/Game%20Design/03_CULTIVATION.md) |
| **Combat & Parry Frame Data** | [`docs/Game Design/04_COMBAT.md`](./docs/Game%20Design/04_COMBAT.md) |
| **World Zones & Qi Density** | [`docs/Game Design/02_WORLD.md`](./docs/Game%20Design/02_WORLD.md) |
| **Code Base & File Layout** | [`docs/Technical Design/21_FOLDER_STRUCTURE.md`](./docs/Technical%20Design/21_FOLDER_STRUCTURE.md) |
| **Database & Persistence** | [`docs/Technical Design/24_SAVE_SYSTEM.md`](./docs/Technical%20Design/24_SAVE_SYSTEM.md) |
| **Network & Hit Registration** | [`docs/Technical Design/23_NETWORK_ARCHITECTURE.md`](./docs/Technical%20Design/23_NETWORK_ARCHITECTURE.md) |
| **Coding Style & Safety** | [`docs/Technical Design/26_CODING_STANDARD.md`](./docs/Technical%20Design/26_CODING_STANDARD.md) |
